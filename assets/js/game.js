...
var fight = function (enemy) {
    ...
    while (enemy.health > 0 && playerInfo.health > 0) {
        if (isPlayerTurn) {
            ...
            //isPlayerTurn true, so player attacks first. 
        ...
        } //player gets attacked first (else=isPlayerTurn is false) 
        else {
...
var endGame = function () {
    window.alert("Game OVER. Let's see how your robot did!");
    debugger;
    //if player is still alive, player WINS
    if (playerInfo.health > 0) {
        window.alert("Great job, you've survived the game! You now have a score of " + playerInfo.money + ". But does it beat the high score?");

        //check the localStorage for a high score. If NULL, use 0.
        var highScore = localStorage.getItem("highscore");
        highScore = highScore || 0;
        //if player money is higher than highScore, player has new high score. 
        if (playerInfo.money > highScore) {
            localStorage.setItem("highscore", playerInfo.money);
            localStorage.setItem("name", playerInfo.name);
            alert(playerInfo.name + " now has the high score of " + playerInfo.money + ". Congrats, baller!");
        }
        else {
            alert(playerInfo.name + " did not beat the high score of " + highScore + ". Big bummer. Sucks to suck, man.");
        }
    }
    else {
...
var shop = function () {
    ...
    var shopOptionPrompt = window.prompt(
        "Would you like to REFILL your health, UPGRADE your attack, or LEAVE the store? Please enter 1 for REFILL, 2 for UPGRADE, or 3 to LEAVE STORE."
    );
